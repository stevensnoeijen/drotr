...
describe('createPendingMoveOrderSystem', () => {
  ...
  it('plans and applies a staged routed order using the grid, handing over a movePath', () => {
    const from = cellPositionToVector(0, 0, CELL_SIZE);
    const destination = cellPositionToVector(4, 4, CELL_SIZE);
    const { self, tick } = setup(
      {
  ...
  it("applies a staged stop order when the destination is the unit's own cell, zeroing velocity and clearing any leftover route", () => {
    const here = cellPositionToVector(2, 2, CELL_SIZE);
    const { self, tick } = setup(
      {
  ...
  it("plans from the unit's position when the order is applied, not its position when it was staged", () => {
    ...
    // eventual route.
    const staleFrom = cellPositionToVector(0, 0, CELL_SIZE);
    const arrivalCell = cellPositionToVector(4, 4, CELL_SIZE);
