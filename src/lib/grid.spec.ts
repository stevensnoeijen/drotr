...
describe('toWorldPositionCellCenter', () => {
  it('should center to grid position', () => {
    const position = toWorldPositionCellCenter(new Vector2(101, 60), CELL_SIZE);
  ...
  it('centers a negative position in its (negative) cell, not the origin cell', () => {
    ...
    const position = toWorldPositionCellCenter(new Vector2(-54, -1), CELL_SIZE);
...
describe('toWorldPosition', () => {
  it('should center to grid position', () => {
    const position = toWorldPosition(new Vector2(10, 2), CELL_SIZE);
...
describe('toGridPosition', () => {
  it('should 0,0 when position is 8,8', () => {
    expect(toGridPosition(new Vector2(8, 8), CELL_SIZE)).toMatchObject({
      x: 0,
      y: 0,
    });
  });
  ...
  it('should round down', () => {
    expect(toGridPosition(new Vector2(130, 130), CELL_SIZE)).toMatchObject({
      x: 4,
      y: 4,
    });
  });
...
describe('screenToWorld -> worldToGrid round trip', () => {
  ...
  it.each([
    ...
    const cell = worldToGrid(world, CELL_SIZE);
    expect(cell).toMatchObject({ x: cellX, y: cellY });

    expect(screenToGrid(screen, viewport, CELL_SIZE)).toMatchObject({ x: cellX, y: cellY });
  });
});

describe('worldToGrid out-of-bounds handling', () => {
  it('returns undefined for a negative cell rather than a negative index', () => {
    const cell = worldToGrid(new Vector2(-10, -10), CELL_SIZE);
    expect(cell).toBeUndefined();
  });

  it('returns undefined when the cell falls outside the given bounds', () => {
    const bounds = { width: 4, height: 4 };
    expect(worldToGrid(new Vector2(4 * CELL_SIZE, 0), CELL_SIZE, bounds)).toBeUndefined();
    expect(worldToGrid(new Vector2(0, 4 * CELL_SIZE), CELL_SIZE, bounds)).toBeUndefined();
    expect(worldToGrid(new Vector2(3 * CELL_SIZE, 0), CELL_SIZE, bounds)).toMatchObject({ x: 3, y: 0 });
    expect(worldToGrid(new Vector2(0, 0), CELL_SIZE, bounds)).toMatchObject({ x: 0, y: 0 });
  });
  ...
  it('screenToGrid returns undefined for a screen position outside the canvas/map', () => {
    ...
    expect(screenToGrid({ x: -50, y: -50 }, viewport, CELL_SIZE)).toBeUndefined();
  });
});
...
describe('cellCentreCoordinate', () => {
  it('centres a column/row index in its cell', () => {
    expect(cellCentreCoordinate(0, CELL_SIZE)).toBe(16);
    expect(cellCentreCoordinate(3, CELL_SIZE)).toBe(112);
  });

  it('agrees with toWorldPosition on both axes', () => {
    const world = toWorldPosition(new Vector2(5, 7), CELL_SIZE);

    expect(cellCentreCoordinate(5, CELL_SIZE)).toBe(world.x);
    expect(cellCentreCoordinate(7, CELL_SIZE)).toBe(world.y);
  });

  it('stays correct for negative indices', () => {
    expect(cellCentreCoordinate(-1, CELL_SIZE)).toBe(-16);
  });
});

describe('isAtCellCentre', () => {
  it('accepts a position exactly on a cell centre', () => {
    expect(isAtCellCentre({ x: 16, y: 16 }, CELL_SIZE)).toBe(true);
    expect(isAtCellCentre({ x: 112, y: 48 }, CELL_SIZE)).toBe(true);
  });

  it('accepts a position within the arrival tolerance of the centre', () => {
    expect(isAtCellCentre({ x: 16 + CELL_CENTRE_TOLERANCE, y: 16 }, CELL_SIZE)).toBe(true);
    expect(isAtCellCentre({ x: 16, y: 16 - CELL_CENTRE_TOLERANCE }, CELL_SIZE)).toBe(true);
  });

  it('rejects a position part-way across its cell on either axis', () => {
    expect(isAtCellCentre({ x: 16 + CELL_CENTRE_TOLERANCE + 0.5, y: 16 }, CELL_SIZE)).toBe(false);
    expect(isAtCellCentre({ x: 16, y: 16 + CELL_CENTRE_TOLERANCE + 0.5 }, CELL_SIZE)).toBe(false);
  });

  it('rejects a position straddling a cell boundary', () => {
    expect(isAtCellCentre({ x: CELL_SIZE, y: CELL_SIZE }, CELL_SIZE)).toBe(false);
  });
  ...
  it('is measured against the cell the position falls in, not the origin cell', () => {
    ...
    expect(isAtCellCentre({ x: -16, y: -16 }, CELL_SIZE)).toBe(true);
    expect(isAtCellCentre({ x: -8, y: -16 }, CELL_SIZE)).toBe(false);
  });

  it('honours an explicit tolerance', () => {
    expect(isAtCellCentre({ x: 20, y: 16 }, CELL_SIZE, 4)).toBe(true);
    expect(isAtCellCentre({ x: 21, y: 16 }, CELL_SIZE, 4)).toBe(false);
  });
});
