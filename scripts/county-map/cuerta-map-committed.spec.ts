...
/**
 * Tests against the **committed** `public/maps/cuerta.tmj` itself. These need
 * no `.cd/` data, so they always run (CI included). That the file is exactly
 * what the converter produces from the real `CUERTA.MAP` is checked by the
 * golden suite (`county-map-golden.spec.ts`), which skips without `.cd/`.
 */

const MAPS_DIR = path.join(process.cwd(), 'public', 'maps');

describe('public/maps/cuerta.tmj', () => {
  const map = JSON.parse(
    fs.readFileSync(path.join(MAPS_DIR, 'cuerta.tmj'), 'utf-8')
  ) as TiledMap;
