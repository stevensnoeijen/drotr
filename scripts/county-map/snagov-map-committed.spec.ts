...
/**
 * Tests against the **committed** `public/maps/snagov.tmj` itself. These need
 * no `.cd/` data, so they always run (CI included). That the file is exactly
 * what the converter produces from the real `SNAGOV.MAP` is checked by the
 * golden suite (`county-map-golden.spec.ts`), which skips without `.cd/`.
 */

const MAPS_DIR = path.join(process.cwd(), 'public', 'maps');

describe('public/maps/snagov.tmj', () => {
  const map = JSON.parse(
    fs.readFileSync(path.join(MAPS_DIR, 'snagov.tmj'), 'utf-8')
  ) as TiledMap;
  ...
  it('passes parseTiledMap as a 128x128, 40 px map with its 6 generated edge spawns', () => {
    ...
    expect(parsed.spawns.map((spawn) => spawn.id)).toEqual(['edge-1', 'edge-2', 'edge-3', 'edge-4', 'edge-5', 'edge-6']);
