...
/**
 * Tests against the **committed** `public/maps/rasova.tmj` itself. These need
 * no `.cd/` data, so they always run (CI included). That the file is exactly
 * what the converter produces from the real `RASOVA.MAP` is checked by the
 * golden suite (`county-map-golden.spec.ts`), which skips without `.cd/`.
 */

const MAPS_DIR = path.join(process.cwd(), 'public', 'maps');

describe('public/maps/rasova.tmj', () => {
  const map = JSON.parse(
    fs.readFileSync(path.join(MAPS_DIR, 'rasova.tmj'), 'utf-8')
  ) as TiledMap;
